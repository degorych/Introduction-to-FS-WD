<?php
$auth = function () use ($connection, $config) {
    define('AUTH', 'auth');
    ...
    if ($nameLen < 3 || $nameLen > 20) {
        $errors[] = createResponse('nonCriticalErr', 'name length error', AUTH, 'Name mast contain from 3 to 20 characters');
    }
    if ($passLen < 3 || $passLen > 20) {
        $errors[] = createResponse('nonCriticalErr', 'password length error', AUTH, 'Password mast contain from 3 to 20 characters');
    }
    ...
    if (empty($findUser)) {
        try {
            ...
            http_response_code(400);
            header('Content-Type: application/json');
            echo json_encode(createResponse('criticalErr', 'bad add user request', AUTH, $e->getMessage()));
            return;
        }
